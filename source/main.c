...
void drawSpriteSheetTile(const Texture texture, const int spriteX, const int spriteY, const int spriteSize,
    ...
    DrawTextureRec(
        texture,
        (Rectangle) { (float)(spriteX * spriteSize), (float)(spriteY * spriteSize), (float)spriteSize* scale.x, (float)spriteSize* scale.y },
        position, WHITE);
}
...
int main(int argc, const char** argv) {
    ...
    InitWindow(initialScreenWidth * 3, initialScreenHeight * 3, "JumpCastle - Raylib by Teamn 3 F-Code");
    ...
    Player player;
    player.position = (Vector2) { (float)initialScreenWidth / (2 * TILE_PIXELS), (float)initialScreenHeight / (2 * TILE_PIXELS) };
    ...
    while (!WindowShouldClose()) {
        ...
        // Draw world to pixelart texture
        {
            ...
            for (int x = 0; x < TILEMAP_SIZE_X; x++) {
                for (int y = 0; y < TILEMAP_SIZE_Y; y++) {
                    ...
                    drawSpriteSheetTile(tilemapTexture, spriteX, spriteY, TILE_PIXELS, (Vector2) { (float)x* TILE_PIXELS, (float)y* TILE_PIXELS }, (Vector2){1, 1});
                }
            }
            ...
            // Draw player, but relative to current screen
            {
                ...
                drawSpriteSheetTile(playerTexture, sprite, 0, 16, Vector2Subtract(worldToScreen((Vector2) { player.position.x, player.position.y - screenOffsetY }), (Vector2) { 8, 10 }), (Vector2) { (float)(player.isFacingRight ? 1 : -1), 1 });
            }
        ...
        // Finalize drawing
        {
            ...
            DrawTexturePro(pixelartRenderTexture.texture, (Rectangle) { 0, 0, (float)pixelartRenderTexture.texture.width, -(float)pixelartRenderTexture.texture.height },
                (Rectangle) { offset.x, offset.y, size.x, size.y }, (Vector2) { 0, 0 }, 0, WHITE);
            ...
            if (isDebugEnabled) {
                ...
                for (int x = 0; x < TILEMAP_SIZE_X; x++) {
                    for (int y = 0; y < TILEMAP_SIZE_Y; y++) {
                        ...
                        DrawTextEx(GetFontDefault(), TextFormat("[%i,%i]\n%i\n\'%c\'", x, y, tile, tile),
                            Vector2Add(worldToScreen((Vector2){ (float)x * scale, (float)y * scale }), Vector2Add(offset, (Vector2) { 3, 3 })),
                            10, 1, RED);
                    }
                }
                ...
                getTilesOverlappedByBox(
                    ...
                    &endX,
                    &endY,
                    (Vector2) { player.position.x, player.position.y - screenOffsetY },
                    PLAYER_SIZE);
